...
namespace Game.Stage
{
   public enum objectType
    ...
    public class StageGenerator : MonoBehaviour
    {
        ...
        [SerializeField] int roomMin = 10;                   // 生成する部屋の最小値,これは部屋の幅、高さの最小値
        ...
        void SplitedRoomSetup()
        {
            ...
            {
                // 変数初期化
                parentNum = -1;  // 分割する部屋番号
                max = 0;        // 最大面積

                // 最大の部屋番号を調べる
                for (int maxCheck = 0; maxCheck < roomNum; maxCheck++)
                {
                    if (roomLocked[maxCheck]) continue;
                    // 面積比較
                ...
                if (parentNum == -1) break;
                //①
                // 取得した部屋をさらに割る
                int dirIndex = SplitPoint(roomStatus[(int)RoomStatus.w, parentNum], roomStatus[(int)RoomStatus.h, parentNum]);

                if (dirIndex == (int)SplitType.cannnotSplit)
                {
                    roomLocked[parentNum] = true;
                    splitNum--;
                    continue;
                }

                if (dirIndex == (int)SplitType.verticle)
                {
                    //横幅がでかい場合
                    // 取得
                ...
                else if (dirIndex == (int)SplitType.horizontal)
                {
                    //横幅の分割の際と同じ
                    //縦幅がでかい場合
                    // 取得
        ...
        void DecideRoomScallAndPosition()
        {
            ...
            for (int i = 0; i < roomNum; i++)
            {
                ...
                int minRx = x + offsetWall;
                int maxRx = minRx < (x + w) - (roomMin + offsetWall) ? (x + w) - (roomMin + offsetWall) : minRx;
                int minRy = y + offsetWall;
                int maxRy = minRx < (y + h) - (roomMin + offsetWall) ? (y + h) - (roomMin + offsetWall) : minRy;
                roomStatus[(int)RoomStatus.rx, i] = Random.Range(minRx, maxRx);
                roomStatus[(int)RoomStatus.ry, i] = Random.Range(minRy, maxRy);
        ...
        void OverrideMap()
        {
            ...
            {
                int rx = roomStatus[(int)RoomStatus.rx, count];
                int ry = roomStatus[(int)RoomStatus.ry, count];
                int rw = roomStatus[(int)RoomStatus.rw, count];
                int rh = roomStatus[(int)RoomStatus.rh, count];

                if (rx < 0 || ry < 0 || rx + rw >= mapSizeW || ry + rh >= mapSizeH)
                {
                    Debug.LogError($"ROOM OUT OF RANGE: {count}, rx={rx}, ry={ry}, rw={rw}, rh={rh} | mapSize={mapSizeW},{mapSizeH}");
                }
                // 取得した部屋の確認
        ...
        private int SplitPoint(int x, int y)
        {
            int minSize = roomMin + offsetWall * 2; // 部屋に必要な最低サイズ
            int required = minSize + offset;        // ランダム余白込み
            int minSplit = required * 2;            // 分割後に両方が部屋を作れるサイズ

            bool canSplitX = x >= minSplit;
            bool canSplitY = y >= minSplit;

            // 横方向優先
            if (canSplitX && x >= y)
            {
                line = Random.Range(required, x - required);
                return (int)SplitType.verticle;
            }
            // 縦方向
            else if (canSplitY)
            {
                line = Random.Range(required, y - required);
                return (int)SplitType.horizontal;
            }
